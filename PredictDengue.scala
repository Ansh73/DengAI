...
object PredictDengue {
  def main(args: Array[String]):Unit = {
    ...
    finaldf_sj.collect().foreach
    {
      ...
      row => positionsArray+=convertRowDataToLabeledPoint(row,fieldNameSeq,row.getAs("total_cases"));
    ...
    finaldf_iq.collect().foreach
    {
      ...
      //We are converting each row to a labeledPoint format.
      row => positionsArray_iq+=convertRowDataToLabeledPoint(row,fieldNameSeq_iq,row.getAs("total_cases"));
  ...
  def convertRowDataToLabeledPoint(rowInput: Row, columnNames: Seq[String], label:Double): LabeledPoint =
  {
    try
    {
      //Get the values of each column.
      val rowValues: Map[String, Double] = rowInput.getValuesMap(columnNames)

      val sortedRowValuesMap = ListMap(rowValues.toSeq.sortBy(_._1): _*)
      val rowValuesItr: Iterable[Double] = sortedRowValuesMap.values
